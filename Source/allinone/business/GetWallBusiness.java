...
import allinone.protocol.messages.GetWallRequest;
import allinone.protocol.messages.GetWallResponse;
 ...
 * @author mcb
 */
public class GetWallBusiness extends AbstractBusiness {

    private static final org.slf4j.Logger mLog =
            LoggerContext.getLoggerFactory().getLogger(GetWallBusiness.class);
    
    
    public int handleMessage(ISession aSession, IRequestMessage aReqMsg, IResponsePackage aResPkg) {
        mLog.debug("get wall");
        MessageFactory msgFactory = aSession.getMessageFactory();
        GetWallResponse resBlogs = (GetWallResponse) msgFactory.getResponseMessage(aReqMsg.getID());

        try {
            
            GetWallRequest rqBlog = (GetWallRequest) aReqMsg;
            CacheWallInfo cache = new CacheWallInfo();
            
            List<AlertEntity> alertFulls = cache.getWall(rqBlog.uid);
...
//            lstUsers.add(currentEntity);
            ...
            if(pageSize == 0)
            {
                pageSize = AIOConstants.MAX_ALERT;
            }
...
//                    usrEntity.mUsername = entity.getUserName();
                ...
                sb.append(Long.toString(entity.getSystemObjectRecordId())).append(AIOConstants.SEPERATOR_BYTE_1);
                sb.append(Long.toString(entity.getRateEntity().getCommentCount())).append(AIOConstants.SEPERATOR_BYTE_1);
                sb.append(Long.toString(entity.getRateEntity().getLikeCount())).append(AIOConstants.SEPERATOR_BYTE_2);
...
//                        imgQueue.insertImage(imgEntity);
                ...
                if(aSession.getByteProtocol()> AIOConstants.PROTOCOL_MXH && rqBlog.pageIndex == 0)
                {
                    ...
                    //add user name
                    sb.append(AIOConstants.SEPERATOR_BYTE_3);
                    sb.append(rqBlog.pageIndex).append(AIOConstants.SEPERATOR_BYTE_1);
...
//                imgEntity.setFileEntity(entity);
            ...
            if(rqBlog.uid != aSession.getUID())
            {
               AuditFollowEntity entity = new AuditFollowEntity(aSession.getUID(), rqBlog.uid);
