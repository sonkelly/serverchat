...
import allinone.protocol.messages.GetTopAlbumRequest;
import allinone.protocol.messages.GetTopAlbumResponse;
 ...
 * @author mcb
 */
public class GetTopAlbumBusiness extends AbstractBusiness {

    private static final org.slf4j.Logger mLog =
            LoggerContext.getLoggerFactory().getLogger(GetTopAlbumBusiness.class);
    
    
    public int handleMessage(ISession aSession, IRequestMessage aReqMsg, IResponsePackage aResPkg) {
        mLog.debug("get friends blog");
        MessageFactory msgFactory = aSession.getMessageFactory();
        GetTopAlbumResponse resBlogs = (GetTopAlbumResponse) msgFactory.getResponseMessage(aReqMsg.getID());

        try {
            
            GetTopAlbumRequest rqAlbum = (GetTopAlbumRequest) aReqMsg;
            
            CacheAlbumInfo cacheInfo = new CacheAlbumInfo();
            
          
            List<AlbumEntity> albums= cacheInfo.getTopAlbums();
